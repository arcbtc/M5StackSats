...
#include "PaymentConnector.h"
  ...
  //API Setup
  String api_key = "<public_api_key>"; // Can be found here: https://lnpay.co/dashboard/integrations
  String wallet_key = "<wallet_access_key>"; // Can be found here: https://lnpay.co/dashboard/advanced-wallets


  //Endpoint Setup
  String api_endpoint = "https://lnpay.co/v1";
  String invoice_create_endpoint = "/wallet/" + wallet_key + "/invoice";
  String invoice_check_endpoint = "/lntx/{{tx_id}}"; //append LNTX ID to the end (e.g. /user/lntx/lntx_mfEKSse22)
  ...
  int PaymentConnector::checkIfPaymentIsSettled(String id) {
    ...
    String payload = PaymentConnector::createRequest((String)"GET",invoice_check_endpoint);
    
    const size_t capacity = JSON_OBJECT_SIZE(2) + 500;
    DynamicJsonDocument doc(capacity);
    deserializeJson(doc, payload);
    
    const char* settled = doc["settled"]; 
    int settledInt = (int) settled;
    if (settledInt == 1){
      return 1;
    } else{
      return 0;
